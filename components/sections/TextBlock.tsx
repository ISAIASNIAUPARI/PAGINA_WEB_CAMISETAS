...
export function TextBlock({
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 items-center gap-8 text-left @3xl:grid-cols-2">
        {(data.image?.url || edit) && (
        ...
        <div className={data.image?.url || edit ? '' : '@3xl:col-span-2'}>
          {paragraphs.map((p) => (
            <p key={p.id} className="mb-4 text-base leading-relaxed" style={{ color: 'var(--color-fog)' }}>
              <EditableText
                as="span"
                edit={edit}
                value={p.text}
